...
public:
	...
	vector<vector<TableData> > daide;
    
	//These datatypes are set to public so that no one can access them without the use of public methods provided for them.
	...
	std::vector<attribute> schema;	
				//Schema is defined as vector of class objects attributes which are defined above which contains attributeName and attributeType
...
public:
	...
	// Returns The schema of the table. Methods to access public Data
	...
	//	showSchemaHeader();
		...
		for(it=dataInTable.begin(); it!=dataInTable.end(); it++)
		{
			...
			for(int j=0;j<noOfAttributes;j++){
				cell currentCell = Tuple[j];
              //  currentCell.printCell();
            // Store the value in a variable
             cellValue.push_back(currentCell.getValueString());
...
int main()
{
	...
	try
	{
		...
		loadData();				//load Data
		cout<<"-----------------------------Welcome to RA Evaluator--------------------------\n\n";
		
		cout<<"Entity: Student				Entity: Teacher				Entity: Course\n";

		cout<<"Column Name	Data Type		Column Name	Data Type		Column Name	Data Type\n";
		cout<<"Name		String			Name		String			CID		Int\n";
		cout<<"Roll		Int			ID  		Int			CName 		String\n";
		cout<<"Sex		Char			DeptID		Int			Faculty		Int\n";
		cout<<"DeptID		Int			Sex		Char			Credits		Int\n";
		cout<<"Grade		Double\n";
		cout<<"isHosteler	Bool\n\n";
		cout<<"The list of the questions for the practice\n";
		cout<<"1.Retrieve Name,Sex,Roll,Grade of students whose roll number is 2 or 7\n";
		cout<<"2.Retrieve the names of male students who are enrolled in the 1001 department and have a grade higher than 7.\n";
		cout<<"3.Retrieve Name,Sex,Roll,Grade of students whose roll number is 9.\n";
		cout<<"4.Display Names and IDs of Students & Teachers.\n";
		cout<<"5.Rename Teacher Entity to Faculty.\n";
		cout<<"6.Retrieve Name,Sex,Roll,Grade of students whose roll number is 8\n";
		cout<<"7.Retrieve Student Name with maximum grade.\n";
		cout<<"8.Retrieve Name,Sex,Roll,Grade of students whose roll number is 11 or 7\n";
		cout<<"9.Retrieve Name,Sex,Roll,Grade of students whose roll number is 5\n";
		cout<<"10.Retrieve Name,Sex,Roll,Grade of students whose roll number is 2 or 5\n";
		cout<<"Enter 0 to quit...\n";
		
		cout<<"Data Loaded..."<<endl;
		admin_answer.push_back("NULL");
		admin_answer.push_back("PROJECT {Name,Sex,Roll,Grade}[SELECT {Roll==2 || Roll==7}[Student]]");
		admin_answer.push_back("PROJECT {Name}[SELECT {Sex=='M' && DeptID==1001 && Grade>7}[Student]]");
		admin_answer.push_back("PROJECT {Name,Sex,Grade}[SELECT {Roll==9}[Student]]");
		admin_answer.push_back("UNION [RENAME{Student|Name,ID}[PROJECT{Name,Roll}[Student]],PROJECT{Name,ID}[Teacher]]");
		admin_answer.push_back("RENAME{Faculty}[Teacher]");
		admin_answer.push_back("PROJECT {Name,Sex,Grade}[SELECT {Roll==8}[Student]]");
		admin_answer.push_back("SDIFF [ PROJECT {Name} [Student] , PROJECT {Name} [  SELECT {Grade < SGrade} [CART [ Student , RENAME {S|Sname,SRoll,SSEX,SDeptID,SGrade,SisHosteler}[Student] ] ] ] ]");
		admin_answer.push_back("PROJECT {Name,Sex,Grade}[SELECT {Roll==11 || Roll==7}[Student]]");
		admin_answer.push_back("PROJECT {Name,Sex,Grade}[SELECT {Roll==5}[Student]]");
		admin_answer.push_back("PROJECT {Name,Sex,Grade}[SELECT {Roll==2 || Roll==5}[Student]]");
		...
		while(1)
		{
			for(int j=0;j<100;j++)
			{
				...
				try
				{
					...
					string answer_query=admin_answer[i];
					...
					{
						Table table1=QueryParser(query);
						vector<vector<string> > ans1=table1.showData();
					
						Table table2=QueryParser(answer_query);
						vector<vector<string> > ans2=table2.showData();
						int flag=0;
						...
						else if (table1.getNoOfAttributes() != table2.getNoOfAttributes() || table1.getNoOfRecords() != table2.getNoOfRecords()) {
						    cout<<"UhOh.... Your query is incorrect.\n";
						    flag=1;
						}
						else	cout<<"UhOh.... Your query is incorrect.\n";
